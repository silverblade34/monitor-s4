...
class EventosResponse:
    def responseDataEventos(self, cod_cuenta, cod_cliente):
        ...
        resp = requests.post(f'http://161.35.104.161:3000/api/v1/Notificaciones/Account', data=json.dumps(data))
        data = resp.json()
    ...
    def responseListarTipoEventos(self):
        resp = requests.get(f'http://192.168.1.44:3000/api/v1/allEvents')
        data = resp.json()
        return data